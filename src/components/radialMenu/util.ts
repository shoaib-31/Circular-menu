...
export function getRingSectionPath(
  ...
  const outerX1 = outerRadius * Math.cos(startAngle + (-0.015));
  const outerY1 = outerRadius * Math.sin(startAngle + (-0.015));
  const outerX2 = outerRadius * Math.cos(endAngle - (-0.015));
  const outerY2 = outerRadius * Math.sin(endAngle - (-0.015));
